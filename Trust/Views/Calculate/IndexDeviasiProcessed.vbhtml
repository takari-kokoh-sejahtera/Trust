...
@Code
    ViewData("Title") = "Calculate Deviasi Processed"
End Code
        ...
        <div class="box">
            <div class="box-header">
                ...
                <div class="box-tools">
                    ...
                    @Using (Html.BeginForm("IndexDeviasiProcessed", ViewData("Title").ToString(), FormMethod.Get))
            ...
            <div Class="box-body table-responsive no-padding">
                <Table Class="table table-hover">
                    <thead>
                        <tr>
                            <th>
                                @Html.ActionLink("Company Group Name", "IndexDeviasiProcessed", New With {.sortOrder = "CompanyGroup_Name", .currentFilter = ViewBag.CurrentFilter, .pageSize = ViewBag.pageSize})
                            </th>
                            <th>
                                @Html.ActionLink("Company_Name", "IndexDeviasiProcessed", New With {.sortOrder = "Company_Name", .currentFilter = ViewBag.CurrentFilter, .pageSize = ViewBag.pageSize})
                            </th>
                            <th>
                                @Html.DisplayNameFor(Function(model) model(0).DeviasiBy)
                            </th>
                            <th>
                                @Html.DisplayNameFor(Function(model) model(0).DeviasiDate)
                            </th>
                            <th>
                                @Html.DisplayNameFor(Function(model) model(0).RemarkDeviasi)
                            </th>
                            <th>
                            ...
                            <th>
                                @Html.ActionLink("Vehicle", "IndexDeviasiProcessed", New With {.sortOrder = "Vehicle", .currentFilter = ViewBag.CurrentFilter, .pageSize = ViewBag.pageSize})
                            </th>
                            <th>
                    ...
                    <tbody>
                        ...
                        @For Each item In Model
                            @<tr>
                                ...
                                <td style="white-space:nowrap">
                                    @Html.DisplayFor(Function(modelItem) item.DeviasiBy)
                                </td>
                                <td style="white-space:nowrap">
                                    @Html.DisplayFor(Function(modelItem) item.DeviasiDate)
                                </td>
                                <td style="white-space:nowrap">
                                    @Html.DisplayFor(Function(modelItem) item.RemarkDeviasi)
                                </td>
                                <td style="white-space:nowrap">
            ...
            <div class="box-footer clearfix">
                <ul class="pagination pagination-sm no-margin pull-right">
                    @Html.PagedListPager(Model, Function(page) Url.Action("IndexDeviasiProcessed", New With {page, .sortOrder = ViewBag.CurrentSort, .currentFilter = ViewBag.CurrentFilter, .pageSize = ViewBag.pageSize}))
                </ul>
            </div>
        </div>
        <!-- /.box -->
