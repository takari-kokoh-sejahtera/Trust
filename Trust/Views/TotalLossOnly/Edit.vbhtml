...
@Using (Html.BeginForm())
    ...
    @<div class="form-horizontal">
        <h4>Tr_TotalLossOnlys</h4>
        <hr />
        @Html.ValidationSummary(True, "", New With {.class = "text-danger"})
        @Html.HiddenFor(Function(model) model.TotalLossOnly_ID)
        ...
        <div class="form-group">
            @Html.LabelFor(Function(model) model.Tolicense_no, "Vehicle From Contract", htmlAttributes:=New With {.class = "control-label col-md-2"})
            <div class="col-md-10">
                @Html.EditorFor(Function(model) model.Tolicense_no, New With {.htmlAttributes = New With {.class = "form-control", .readonly = "readonly"}})
                @Html.ValidationMessageFor(Function(model) model.Tolicense_no, "", New With {.class = "text-danger"})
            </div>
        </div>
